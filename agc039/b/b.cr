n = read_line.to_i
s = (1..n).map{|a|read_line.chomp.chars.map(&.to_i)}
#######
result = -1
nodes = Array(Hash(Symbol , (Array(Int32)? | Bool | Nil) ) ).new(n+1){
  {
    :sides => Array(Int32).new(0),
    :limit => nil, # nilはallOK, []はallNG
    :kotei => nil, # 固定したら
    :visit => false,
  }
}
p typeof(nodes[1][:sides])
comment = <<-EOS
...
n.times do |i|
  n.times do |ii|
    nodes[i+1][:sides].push(ii+1) if s[i][ii] == 1
  end
end
